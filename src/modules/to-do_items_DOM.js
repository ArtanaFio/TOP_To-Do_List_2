...
export function applyExistingTasks(container, backendTask, convertDate) {
  ...
  const title = makeElement(
    "p",
    "",
    "task-text title-id existing-property",
    backendTask.title,
    mainBox
  );
  ...
  const hiddenDetailsBox = makeElement(
    "div",
    "",
    "hidden-box gone",
    "",
...
export function editFrontendTask(
  frontendTask,
  newDescription,
  newPriority,
  editForm,
  ...
  if (convertCalendarDate(editForm.dueDateDropDownBox.value) !== "") {
    frontendTask[3].textContent = convertCalendarDate(
